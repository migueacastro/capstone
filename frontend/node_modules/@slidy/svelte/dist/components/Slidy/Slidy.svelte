...
<script>
	...
	export let getThumbSrc = (item) => getImgSrc(item);
...
>
	...
	{#if arrows}
		<slot name="arrows">
			{#each [-1, 1] as direction}
				<Arrow {direction} {index} items={length} {loop} step={clamp > 0 ? clamp : 1} {vertical}>
					<slot name="arrow">
						<svg class="slidy-arrow-icon" viewBox={iconChevron.viewBox} xmlns="http://www.w3.org/2000/svg">
	...
	{#if progress}
		<Progress value={index + 1} max={length} {vertical} on:input={(e) => (index = e.currentTarget.valueAsNumber - 1)} />
	{/if}
